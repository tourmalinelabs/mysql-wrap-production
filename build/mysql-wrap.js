...
const createMySQLWrap = (poolCluster, options, connection) => {
    ...
    const mapOrderBy = raw => _.map(_.isArray(raw) ? raw : [raw], o => _.isString(o)
        ? {
            field: o,
            isAscending: true,
        }
    ...
    self.encodeCursor = (orderByRaw, row) => {
        ...
        return new Buffer(_.map(orderBy, o => o.serialize ? o.serialize(row[o.field]) : String(row[o.field])).join(CURSOR_DELIMETER)).toString('base64');
    };
    const runCursor = (q, fig) => {
        const orderBy = mapOrderBy(fig.orderBy);
        const isAscending = fig.last && !fig.first ? false : true;
        const decodeCursor = c => _.map(new Buffer(c, 'base64').toString('ascii').split(CURSOR_DELIMETER), (v, i) => (orderBy[i].deserialize ? orderBy[i].deserialize(v) : v));
        const buildWhereArgs = (values, isGreaterThan) => {
            const build = (values, orderBy, isGreaterThan) => {
                const sql = _.map(orderBy, (o, i) => i === values.length - 1
    ...
    self.query = (statementRaw, values) => {
        ...
        return getConnection(isSQLReadOrWrite(statementObject.sql)).then((conn) => new Promise((resolve, reject) => {
