  ...
  describe("build", () => {
    before((done) => {
      ...
      self.toCursor = (r, fields) =>
        new Buffer(_.map(_.isArray(fields) ? fields : [fields], (f) => String(r[f])).join("#")).toString("base64");
  ...
  describe("query", () => {
    ...
    it('should have table/field parameters using "??"', (done) => {
      self.sql
        ...
        .then((rows) => {
          chai.assert.sameDeepMembers(rows, [{ unique: "a" }, { unique: "b" }, { unique: "c" }]);
          done();
        })
        .done();
    });
  ...
  describe("save", () => {
    it("should insert row if does not exist", (done) => {
      self.sql
        ...
        .then((res) => {
          ...
          self.masterConn.query("SELECT * FROM `table` WHERE `id` = 4", (err, res) => {
            chai.assert.deepEqual(res, [{ id: 4, unique: "d", field: "baz" }]);
            done();
          });
        })
        .done();
    ...
    it("should update row if exists by unique constraint", (done) => {
      self.sql
        ...
        .then(() => {
          self.masterConn.query('SELECT * FROM `table` WHERE `unique` = "c"', (err, res) => {
            chai.assert.deepEqual(res, [{ id: 3, unique: "c", field: "update" }]);
            done();
          });
        })
        .done();
